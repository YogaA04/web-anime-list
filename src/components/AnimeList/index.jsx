...
const AnimeList = ({ api }) => {
    return (
        <div>
            <div className="grid md:grid-cols-7 sm:grid-cols-4 grid-cols-2 gap-4">
                {api.data?.map((anime) => {
                    return (
                        <Link href={`/anime/${anime.mal_id}`} className="click relative" key={anime.mal_id}>
                            <Image src={anime.images.webp.image_url} alt="..." width={350} height={350} className="w-full max-h-[300px] object-cover rounded" />
